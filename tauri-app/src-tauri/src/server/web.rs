...
fn is_valid_origin(origin: Option<&str>) -> bool {
    match origin {
        None => true,
        Some(o) => crate::network::is_lan_origin(o),
    }
}
...
impl WebServer {
    ...
    pub fn stop(&self) {
        // Attempt to cancel; if the lock is poisoned we still mark as not running.
        match self.cancel_token.lock() {
            Ok(token_guard) => token_guard.cancel(),
            Err(e) => log::error!(target: "web", "Web server cancel_token mutex poisoned: {e}"),
        }
        self.running.store(false, Ordering::SeqCst);
        // Do NOT reset client_count — active WebSocket handlers will
        // decrement it naturally as they drain.
    }
}
