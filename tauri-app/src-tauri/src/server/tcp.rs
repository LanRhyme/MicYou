...
use micyou_protocol::frame;
use micyou_protocol::micyou::{AudioPacketMessageOrdered, MessageWrapper};
use micyou_protocol::{DeviceInfo, HANDSHAKE_CLIENT_STR, HANDSHAKE_SERVER_STR, PACKET_MAGIC};
use std::error::Error;
...
pub async fn start_tcp_server(
    ...
    log::info!(target: "tcp", "TCP Control Server listening on {}:{}", bind_address, port);

    loop {
        tokio::select! {
            _ = cancel_token.cancelled() => {
                log::info!(target: "tcp", "TCP Server cancelled");
                break;
            }
            accept_result = listener.accept() => {
                match accept_result {
                    Ok((socket, addr)) => {
                        log::info!(target: "tcp", "New client connected: {}", addr);
                        ...
                        tokio::spawn(async move {
                            if let Err(e) = handle_client(socket, addr, app_handle_clone.clone(), audio_tx_clone, stats_clone, mode_clone, connection_tx_clone, active_handle_clone).await {
                                log::error!(target: "tcp", "Client {} error: {}", addr, e);
                            }
                            // Clear handle on disconnect
                            active_handle_cleanup.lock().await.take();
                            log::info!(target: "tcp", "Client {} disconnected", addr);
                            let _ = app_handle_clone.emit("device-disconnected", ());
                        });
                    }
                    Err(e) => {
                        log::error!(target: "tcp", "Failed to accept TCP connection: {}", e);
                    }
                }
            }
        }
...
pub fn force_close_socket(raw: RawSocketHandle) {
    if let Err(e) = safe_shutdown_socket(raw) {
        log::error!(target: "tcp", "Force close socket error: {}", e);
    }
}
...
async fn handle_client(
    ...
    if handshake_buf != HANDSHAKE_CLIENT_STR {
        log::error!(target: "tcp", "Invalid handshake from client: {:?}", handshake_buf);
        return Err("Invalid handshake from client".into());
    }

    // Send Server Handshake
    socket.write_all(HANDSHAKE_SERVER_STR).await?;
    log::info!(target: "tcp", "Handshake successful with {}", addr);
    ...
    let current_time = crate::util::now_millis() as u64;
    ...
    let writer_task = tokio::spawn(async move {
        while let Some(msg) = rx.recv().await {
            let frame = frame::encode_frame(&msg, PACKET_MAGIC);
            if let Err(e) = write_half.write_all(&frame).await {
                log::error!(target: "tcp", "Write failed: {}", e);
                break;
            }
        }
    });
    ...
    loop {
        ...
        loop {
            match frame::decode_frame(&buffer, PACKET_MAGIC) {
                Ok((message, consumed)) => {
                    buffer.advance(consumed);
                    handle_message(message, &tx, &audio_tx, &stats, &app_handle).await?;
                }
                Err(frame::FrameError::Incomplete) => break, // Need more data
                Err(frame::FrameError::InvalidMagic) => {
                    writer_task.abort();
                    ping_task.abort();
                    monitor_task.abort();
                    log::error!(target: "tcp", "Invalid packet magic received");
                    return Err("Invalid packet magic".into());
                }
                Err(frame::FrameError::DecodeError(e)) => {
                    log::error!(target: "tcp", "Failed to decode packet: {}", e);
                    continue;
                }
            }
        }
...
async fn handle_message(
    ...
    if let Some(mute) = msg.mute {
        log::info!(target: "tcp", "Received mute state: {}", mute.is_muted);
