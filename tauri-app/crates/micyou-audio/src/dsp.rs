...
impl DpdfnetProcessor {
    ...
    fn process(&mut self, input: &[f32]) -> Vec<f32> {
        ...
        let val_spec =
            ort::value::Value::from_array((vec![1, 1, spec_size, 2], spec_flat)).unwrap();
        let val_state =
            ort::value::Value::from_array((vec![56436], self.state_data.clone())).unwrap();
