...
import com.lanrhyme.micyou.settings.AppSettings
...
class UpdateViewModel : ViewModel() {
    ...
    init {
        viewModelScope.launch {
            updateChecker.downloadProgress.collect { p ->
                _uiState.update {
                    it.copy(
                        updateDownloadProgress = p.progress,
                        updateDownloadedBytes = p.downloadedBytes,
    ...
    fun downloadAndInstallUpdate(useMirror: Boolean) {
        ...
        val targetUrl =
            if (useMirror) info.mirrorUrl else info.githubRelease?.let { updateChecker.findAssetForPlatform(it)?.browserDownloadUrl }
        if (targetUrl == null) {
            info.githubRelease?.htmlUrl?.let { openUrl(it) }
            return dismissUpdateDialog()
        }
        val qName = Regex("(?i)[?&](?:filename|file|name)=([^&]+)").find(targetUrl)?.groupValues?.get(1)
            ?.substringAfterLast("/")
        val pName = targetUrl.substringBefore("?").substringAfterLast("/").takeIf { it.contains(".") }
        val ext = pName?.substringAfterLast(".", "") ?: qName?.substringAfterLast(".", "")
        ?: info.githubRelease?.let { updateChecker.findAssetForPlatform(it)?.name }?.substringAfterLast(".", "")
        ?: "apk"
        val name = pName ?: qName?.takeIf { it.contains(".") }
        ?: "MicYou-${info.versionName}-android-${getMirrorArch()}.${ext.takeIf { it.isNotBlank() } ?: "apk"}"
    ...
    private fun failDownload(error: String?) =
        _uiState.update { it.copy(updateDownloadState = UpdateDownloadState.Failed, updateErrorMessage = error) }

    fun dismissUpdateDialog() = _uiState.update { UpdateUiState() }

    fun openGitHubRelease() {
        openUrl(
            _uiState.value.updateInfo?.githubRelease?.htmlUrl ?: "https://github.com/LanRhyme/MicYou/releases/latest"
        )
        dismissUpdateDialog()
    }
}
