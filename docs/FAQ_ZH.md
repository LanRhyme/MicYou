...
1. **确认防火墙设置**
   ...
   Windows 防火墙可能会拦截入站连接。请按照以下方法手动放行端口：

   1. 按下 `Win+R`，输入 `powershell`，同时按住 `Ctrl+Shift`，点击「确定」以管理员身份运行 PowerShell。
   2. 输入以下命令：
      ...
      > MicYou 默认使用 TCP 端口 `6000`（控制通道）和 UDP 端口 `6001`（音频数据）。如已修改端口号，请将命令中的端口号替换为实际值。

      若未出现任何错误提示，表示操作成功，可以重新尝试连接。
